...
| CovePM Marketing Daily Brief | Weekdays at 7:30 AM America/New_York | Summarize the previous day's marketing-agent sessions | Active | 2026-09-21 | Next weekday run | Automation ID: `covepm-marketing-daily-brief`; output in `Sessions/` |
| Local marketing workbench | Every 2 hours on weekdays, 8:00 AM–6:00 PM | Scan queue -> local model -> QA -> approval -> control-room snapshot -> mobile run report and approval inbox | Active local MVP | 2026-09-21 | Next Task Scheduler run | `agent-workbench/run-weekly.ps1`; processes one queued task per run, syncs results, and sends deduplicated pending approvals to the private mobile app |
| LinkedIn publishing and engagement workflow | Weekdays at 8:30 AM and 2:30 PM; engagement at 10:30 AM and 4:00 PM | Public social listening -> prepare two posts plus two engagement batches for Publora | Active, approval-gated | 2026-09-20 | Next weekday run | Research must be current within 14 days; see [[LinkedIn Publishing and Engagement Workflow]]; no external action without human approval |
